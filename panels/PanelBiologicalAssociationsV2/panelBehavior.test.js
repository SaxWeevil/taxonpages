...
test('Standard shows only evidence rows until the switch widens it', async t => {
  ...
  const basic = [
    { id: 1, subject_otu_id: 11, object_otu_id: 21, relationship: 'undefined relationship with',
      subject: { type: 'AnatomicalPart', id: 95, label: 'larvae: Hypera test', family: 'Curculionidae' },
      object: { type: 'Otu', id: 21, label: 'Plant one', family: 'Apiaceae' } },
    { id: 2, subject_otu_id: 11, object_otu_id: 22, relationship: 'collected from',
      subject: { type: 'Otu', id: 11, label: 'Hypera test', family: 'Curculionidae' },
      object: { type: 'Otu', id: 22, label: 'Plant two', family: 'Apiaceae' } },
    { id: 3, subject_otu_id: 11, object_otu_id: 23, relationship: '[legacy] feeds on',
      subject: { type: 'Otu', id: 11, label: 'Hypera test', family: 'Curculionidae' },
      object: { type: 'Otu', id: 23, label: 'Plant three', family: 'Apiaceae' } }
  ]
  const response = data => ({ data, headers: { 'pagination-page': '1',
    'pagination-per-page': String(data.length || 1), 'pagination-total': String(data.length) } })
  const state = await createPanel(t, { get: async (url, config = {}) => {
...
test('a Records drilldown out of Standard is not narrowed again by the relationship filter', async t => {
  ...
  state.summaryAsSubjectRows.value = [
    ...
    { id: 42, relationship: 'reared from',
      subject: { type: 'Otu', id: 11 }, object: { type: 'Otu', id: 22 } }
  ]
  // A Raw-data selection that would otherwise drop the clicked record.
  state.selectedRelationships.value = ['reared from']
  state.standardReady.value = true
  ...
  const query = new URLSearchParams(
    calls.find(url => url.startsWith('/biological_associations?')).split('?')[1])
  assert.deepEqual(query.getAll('biological_association_id[]'), ['41'])
  assert.equal(query.has('biological_relationship_id[]'), false)
